...
import KingdomMap from '../../components/gamification/KingdomMap.jsx';
import QuestionRenderer from '../../components/quiz/QuestionRenderer.jsx';
import BossBattleModal from '../../components/quiz/BossBattleModal.jsx';
      ...
      <div className="play-done-wrap">
        <div className="play-done-card glass-card anim-bounce-in">
          ...
          <div className="play-done-stats">
            ...
            <div className="stat-pill"><span>⭐</span><span>{state.xp} XP Earned</span></div>
    ...
    return (
      <div className="play-map-wrap">
        <div className="play-map-card glass-card">
          <h2 className="play-map-title subheadline">🗺️ PictoQuest Worlds Kingdom</h2>
          <p className="body-text" style={{ color: 'var(--text-secondary)', textAlign: 'center' }}>
            {isAllDone ? (
              ...
              <>District {distIdx + 1}: <strong style={{ color: 'var(--gold)' }}>{district.name}</strong></>
            )}
          </p>
          ...
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap', marginTop: '10px' }}>
            {!isAllDone ? (
              <>
                ...
                <button className="btn-outline" onClick={() => setShowBoss(true)} style={{ borderColor: '#feca57', color: '#feca57' }}>
                  👑 Challenge Boss ({district.boss.name})
                </button>
                <button className="btn-outline" onClick={() => dispatch({ type: 'SET_PHASE', payload: 'reflect' })}>
                  📓 Jump to Reflect
                </button>
              </>
            ) : (
